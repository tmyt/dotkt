...
class CSharpCodegen {
	...
	fun generateFile(file: IrFile): String {
		sb.setLength(0)
		indent = 0
		// User-declared classes (not @Clr façades, not annotations) become C# classes.
		val classes = file.declarations.filterIsInstance<IrClass>()
			.filter { clrName(it) == null && it.kind == ClassKind.CLASS }
		val objects = file.declarations.filterIsInstance<IrClass>()
			.filter { clrName(it) == null && it.kind == ClassKind.OBJECT && !it.isCompanion }
		val functions = file.declarations.filterIsInstance<IrSimpleFunction>()

		if (classes.isEmpty() && objects.isEmpty() && functions.isEmpty()) return ""
	...
	private fun generateClass(klass: IrClass) {
		val base = baseClassOf(klass)
		val baseClause = base?.let { " : " + (clrName(it)?.let { n -> "global::$n" } ?: it.name.asString()) } ?: ""
		val abstract = if (klass.modality == Modality.ABSTRACT) "abstract " else ""
		line("public ${abstract}class ${klass.name.asString()}$baseClause")
		line("{")
		indent++
	...
	private fun IrClass.memberMethods(): List<IrSimpleFunction> =
	...
	/** The user-declared class a Kotlin class extends, if any (ignores Any and interfaces). */
	private fun baseClassOf(klass: IrClass): IrClass? = klass.superTypes
