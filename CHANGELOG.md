...
- **Nested projections retain reifiable physical types (#844).** A projection consumed by a non-generic CLR
  classifier no longer erases its enclosing collection, including shared arguments captured by suspend defaults.
...
- **Existential Collection Count follows its independent collection contract (#782).** List/Set views
  ...
  MutableCollection Count keeps its mutable contract, and List Count/Get prefer their generic view over raw storage.
...
- **Reified Collection and Set classifiers preserve their families (#785).** Tests and casts reject unrelated
  ...
  Raw collections retain Collection membership, while concrete MutableCollection<Any/Any?> casts require their
  actual typed CLR interface rather than treating the concrete element type as a star projection.
...
- **Inherited final methods preserve their CLR names when satisfying interfaces (#718).** Forwarders retain
  ...
  selected implementation and getter/setter association; source generic bounds remain distinct from CLR constraints.
...
- **Projected collection operations now retain an existential CLR receiver (#673).** bir2cir routes collection
  ...
  generic collection face. Star-projected iterable, collection, set, and list values crossing an ordinary covariant
  call boundary also receive a live view when CLR value-type variance cannot represent the legal Kotlin edge.
...
- **Projected mutable-collection adds now use the receiver's actual CLR collection face (#671).** Calls through
  ...
  result and resolves the runtime object's exact closed collection interface.
