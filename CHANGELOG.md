...
- **Foreign nullable-value arrays now fail with their exact uninhabitable crossing (#354).** A .NET member declaring
  `Nullable<V>[]` is rejected at parameter or return use with an actionable diagnostic that names both the declared
  array and Kotlin's canonical `object[]` physical image, rather than describing the crossing only through a generic
  collection example.
