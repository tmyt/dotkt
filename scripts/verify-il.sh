...
il_check dimgrandchild AppKt "$ROOT/cases/il-dimgrandchild/app.kt" "$(printf 'square area=16\nsquare area=9\nshape area=7\ncircle area=5\npentagon area=11')"   # #185: an interface DIM overridden by a GRANDCHILD (intermediate class does NOT override) must dispatch to the override, not the DIM default — ilemit wires a per-type MethodImpl to the base-inherited interface slot (ECMA-335 II.12.2)
