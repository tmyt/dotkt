...
il_check nncontract NnContract "$ROOT/cases/il-nncontract/app.kt" "$(printf '2\ntb\nnpe-param\nnpe-ctor\nnpe-member')"   # #6: non-null PARAMETER PRECONDITIONS on the public surface (top-level fun / ctor / member fun) throw NullPointerException fail-fast when a laundered null crosses the boundary; a normal non-null call is unaffected
