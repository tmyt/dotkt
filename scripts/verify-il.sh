...
declare -A XFAIL_RUN=(
	[collops2]="TWO stacked bugs behind listOf(...).windowed(3). BUG #1 (FIXED by the ilemit ctor-anchor commit): a generic class's 'constructor(...) : this(...)' delegation was emitted on the OPEN generic type DEFINITION instead of the constructed self-instantiation C<T> (ilemit EmitCtorBody, thisArgs branch). The stdlib RingBuffer<T>(capacity) secondary ctor delegates that way, and ONLY windowed's step<size path (gap=step-size<0) constructs a RingBuffer, so it alone hit System.InvalidOperationException 'not fully instantiated' (re-thrown at ResultKt.throwOnFailure). chunked / windowed(n,n,true) / windowed(3,3,false) use step==size -> gap>=0 -> the eager ArrayList branch, never RingBuffer, so they always passed. Fixed by anchoring the sibling ctor via TypeBuilder.GetConstructor over the self-instantiation; isolated regression gate = il-genctor; the whole rt stdlib re-emits clean. REMAINING BLOCK = BUG #2 (residual, coroutine bundle, NOT an ilemit ctor/Result issue): after #1, windowed(3)'s gap<0 path advances into the cold-sequence SM and dies with a NullReferenceException inside kotlin.coroutines.clr.internal.BaseContinuationImpl.resumeWith -> SequenceBuilderIterator.resumeWith -> invokeSuspend (same family as sequence { yield(1) }.toList()), captured and re-thrown at throwOnFailure. This is the generic cold-sequence SM gap (bir2cir SuspendColdLowering + ilemit generic-SM codegen); windowed(3,3,false) [gap==0, ArrayList] still works, proving the two-bug split."
)
