...
il_check genarrlam GenArrLam "$ROOT/cases/il-genarrlam" "$(printf '2\n3')"  # #142: `Array(size){ mk<T?>(null) }` in a generic class — nested constructed-generic `Ref<T?>` erased to `Ref<object>` CONSISTENTLY across method-sig/array-elem/newDelegate.funcType.ret (EraseNullableTv Fn carve-out narrowed to top-level Nullable(Tv)); DelegateCtor gone
...
if [[ -n "$ILV" && -d "$REFDIR" ]]; then
	...
	ASMS+=( [alias]=Alias [blank]=Blank [cbk]=Cbk [clrasm]=ClrAsm [clriface]=ClrIface [clrimpl]=ClrImpl [coerce]=Coerce [coevalorder]=CoEvalOrder [cofieldorder]=CoFieldOrder [cofinally]=CoFinally [coldabstract]=ColdAbstract [ifacesuspend]=IfaceSuspend [counit]=CoUnit [delegatearg]=Dlg [dualrep]=DualRep [eventext]=EventExt [genclosure]=GenClo [geninlinearg]=GenInlineArg [genextnew]=GenExtNew [gencolladd]=GenCollAdd [genhof]=XHof [genim]=GenIM [geninj]=GenInj [ifaceevent]=AppKt [inherit]=Inherit [interpnull]=InterpNull [mref]=Mr [netattr]=NetAttr [netenum]=NetEnum [netinterop]=NetInterop [ntostr]=NToStr [nulltostr]=NullToStr [objgen]=OGen [outref]=Outref [overrideprop]=OverridePropKt [regexreplace]=RegexReplace [revinterop]=KotlinLib [samcmp]=SamCmp [selfref]=SelfRef [seqfilter]=SeqFilter [subseq]=SubSeq [taskgen]=Tg [tloverload]=TlOverload [transinj]=TransInj [vtprop]=VtProp [xfaceimpl]=XFace [xinline]=XInl [strops]=StrOps [nullbang]=NullBang [tryval]=TryVal [starproj]=StarProj [toplateinit]=TopLateinit [gendelegate]=AppKt [csextrecv]=CsExtRecv )
