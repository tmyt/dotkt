...
declare -A XFAIL_RUN=(
	...
	[chunk]="bundle-6 P5: cold-core sequences RUN (chunk output correct); the tail filterNotNull() on List<Int?> NREs on the value-type-nullable T?->T drop. kotc emits the nullable:gp:T type-arg marker on the Iterable<T?> receiver AND now the sibling nullable:true on filterNotNullTo's 'var element: T?' LOCAL; PENDS the bir2cir general marked-local erasure (var element type stays gp:T in CIR -> forEachInline/next() unboxes a null Nullable<Int> to Int). Same bir2cir gap as seq"
	...
	[seq]="bundle-6: value-type Sequence chains (map/filter/take/takeWhile/dropWhile/first/count/sum) now RUN — the FilteringSequence value-type crash is FIXED. Remaining fail is the LAST line only: single{} terminal's value-type-nullable LOCAL 'var single: T? = null'. kotc NOW emits the sibling nullable:true marker on the local (BirEmitterStatements var path); PENDS the bir2cir general marked-local erasure (EraseNullableGpDecls only covers fields/props; RetypeGetterReaderVars only getter-reader locals) — the CIR keeps type=gp:T so 'single as T' NREs on the value-type null"
...
declare -A XFAIL_ILVERIFY=(
	...
	[chunk]="ilverify counterpart of the already-XFAIL'd run:chunk value-type-nullable P5 finding (StackUnexpected: IReadOnlyList<Nullable<int32>> where IEnumerable<object> expected — the List<Int?> filterNotNull marker erasure PENDS the bir2cir consumer, sibling P5). Baseline sync: main already carries this XFAIL (958e1bc), my worktree base (74fb4b0) predated it; not touched by this change"
