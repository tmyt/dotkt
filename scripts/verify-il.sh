...
declare -A XFAIL_RUN=(
	...
	[taskgen]="bundle-6 ②: kotc-side DONE (companion generic-static builder resolves Task.FromResult<Int>(42) -> clrGenericStatic; E2E-verified '42' with a hand-authored generic sfun meta). BLOCKED on facadegen surfacing the generic static: it skips m.IsGenericMethod at facadegen/Program.cs:557 (must emit \`sfun FromResult generic:Task1[TResult] TResult value:TResult\` — bare type-param tokens + return/param mapped against them, mirroring fun/tlfun). Auto-passes once facadegen emits it"
...
# seqyieldall: bir2cir SuspendColdLowering BUG Y — the cold-call `sig` disambiguates SequenceScope.yieldAll's
# three same-named `$dotkt_suspend` overloads (see XFAIL_RUN[seqyieldall] for the pending ilemit companion).
