...
# valueawait (valueawait_valueTaskAwaiterFastPath): migrated -> tests/coroutines/fixtures/CorBTaskAwaitTests.kt (#10:
# `await` generalized to a NON-Task BCL awaitable ValueTask<Int>, ValueTaskAwaiter sync fast path); case dir + line removed same-change.
...
# coroutinectx (coroutinectx_currentContinuationContextRead): migrated -> tests/coroutines/fixtures/CorBSuspendCoreTests.kt
# (#79 `coroutineContext` read bound to <current continuation>.get_context(): SM-body / body-direct / member shapes); case dir + line removed same-change.
...
# ifacesuspend (ifacesuspend_virtualInterfaceSuspendMember): migrated -> tests/coroutines/fixtures/CorBSuspendValueTests.kt
# (interface `suspend fun` member cold-entry synthesis + virtual dispatch to the override); case dir + line removed same-change.
...
# seqyieldall (seqyieldall_yieldAllOverloadPick): migrated -> tests/coroutines/fixtures/CorBSequenceTests.kt (BUG Y:
# cold-call `sig` disambiguates SequenceScope.yieldAll's three same-named `$dotkt_suspend` overloads); case dir + line removed same-change.
...
# suspendcatch (suspendcatch_hoistedSuspendingCatch): migrated -> tests/coroutines/fixtures/CorBSuspendCoreTests.kt (#78 Defect B: HoistSuspendingCatches lifts a suspending catch handler out of the CLR clause; two-level dispatch + multi-catch); case dir + line removed same-change.
# suspendintrinsic (suspendintrinsic_intrinsicReadInSuspendBlock): migrated -> tests/coroutines/fixtures/CorBIntrinsicTests.kt (#80: direct read of top-level val COROUTINE_SUSPENDED canonicalized to the SM's Suspended() marker in Rewrite); case dir + line removed same-change.
# suspendintrinsicowned (suspendintrinsicowned_nonSuspendMemberReadIdentity): migrated -> tests/coroutines/fixtures/CorBIntrinsicTests.kt (#157: a NON-suspend member reads top-level val COROUTINE_SUSPENDED, bound via the GENERAL owner-null resolver -> IntrinsicsKt); case dir + line removed same-change.
# suspendloop (suspendloop_splliceLocalSpillAcrossResume): migrated -> tests/coroutines/fixtures/CorBSuspendCoreTests.kt (#82: FlattenSuspendingLoops flattens a suspending forArray/forEachInline loop to CFG so temps/element cross the resume as SM fields; + break/continue); case dir + line removed same-change.
# inline-suspend (inlineSuspend_suspendCallInNonSuspendInlineArgLambda): migrated -> tests/coroutines/fixtures/CorBInlineSuspendTests.kt
# (#75 S4a §8.7 arm-c: a suspend call inside a NON-suspend-typed inline-arg lambda splices into work()'s SM); case dir + line removed same-change.
# suspendnestedcapture (suspendnestedcapture_nestedClosureInCarrier): migrated -> tests/coroutines/fixtures/CorBSuspendCoreTests.kt
# (#22: §4.4ii MaterializeCarrier allows a nested newClosure in the carrier capturing cont/local/loop-binder; the
# suspendCancellableCoroutine{cont->cont.invokeOnCancellation{}} shape); case dir + line removed same-change.
...
# safecontresume (safecontresume_crossThreadResumeCAS): migrated -> tests/coroutines/fixtures/CorBSuspendCoreTests.kt
# (#142: a SafeContinuation resumed from a worker thread — UNDECIDED->SUSPENDED/RESUMED race closed by the Interlocked CAS); case dir + line removed same-change.
...
# lam1 (lam1_bareSuspendLambda) + lam2 (lam2_capturingSuspendLambda): migrated -> tests/coroutines/fixtures/CorBSuspendValueTests.kt
# (bundle-6 P3 wave-2b suspend-LAMBDA payoff: bare `blockOn { 42 }` + a capturing suspend lambda with a real suspend call); case dirs + lines removed same-change.
