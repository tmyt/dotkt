...
declare -A XFAIL_RUN=(
	[collops2]="COROUTINE cold-sequence bug (generic context) — NOT the cross-module default-arg problem the old label claimed. windowed(3) drives the stdlib windowedIterator over a SequenceBuilderIterator whose next() NREs when the cold-sequence SM is instantiated inside a GENERIC method (minimal repro: fun T wrap(x:T)=sequence{yield(x)}.toList() NREs at SequenceBuilderIterator.next; a concrete/reference element works). The general generic-class secondary-ctor delegation bug this XFAIL ALSO masked (RingBuffer generic : this(...) -> open ctor not-fully-instantiated) is FIXED (il-genctor, ilemit ctor-anchor). Remaining = generic cold-sequence SM init/drive (bir2cir suspend + ilemit generic-SM); in flight."
)
