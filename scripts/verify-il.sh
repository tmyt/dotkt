...
declare -A XFAIL_RUN=(
	...
	[digittoint]="bundle-6 ④: Char.digitToIntOrNull() -> InvalidProgram even for '7' (kotlin.text.CharKt.digitToIntOrNull is structurally invalid IL). ROOT CAUSE is in kotc, NOT ilemit: the inlined takeIf lowers to an if-expression whose Kotlin type is Int?, but BirEmitter.ternary() (BirEmitter.kt:3033-3044) tags the cond node type = birType(node.type) = kotlin.Int (the IrWhen .type is the non-null Int; the T? nullability rides the fn return, which IS correctly nullable:int). So the cond has a value-type result type but an else-branch that is a kotlin.Nothing null literal -> after bir2cir the CIR cond is {type:int, then:int-local, else:const object null}. ilverify: PathStackUnexpected [found Int32][expected Nullobjref] at digitToIntOrNull(char)/0x1D. ilemit's EmitCond/EmitNullableCoerced/EmitReturnCoerced already emit correct Nullable<int> IL IF the cond is tagged nullable:int (as kotc DOES for bir-kgenseq/bir-safecallnv); the fix is kotc tagging the cond nullable when a branch is a null literal. ilemit must NOT re-derive the value-type+null join (Codex-confirmed layer boundary: cond result type must be nullable BEFORE CLR lowering)."
