...
declare -A XFAIL_RUN=(
	...
	[collops2]="bundle-6 P5 PARTIALLY FIXED: the ilemit cross-module default-argument drop (windowed(3)) is FIXED — EmitArgsTyped now backfills omitted trailing [Optional]/[DefaultParameterValue] args from the resolved MethodInfo, exactly like EmitCallArgs, so windowed is now called with all 4 args and the app IL is ilverify-CLEAN (collops2 pruned from XFAIL_ILVERIFY). REMAINING run-fail is a SEPARATE, deeper bir2cir defect the default-fix unmasked: windowed's List-returning impl drives a coroutine sequence (windowedIterator) whose SequenceBuilderIterator.resumeWith -> ResultKt.getOrThrow<T> -> throwOnFailure crashes with 'not fully instantiated'. ROOT (bir2cir star-projection): the CIR for getOrThrow<T> lowers `Result<*>.throwOnFailure()` to `callStatic throwOnFailure sig=@kotlin.Result[object]` receiving `local __self : @kotlin.Result[gp:T]` with NO coercion — passing Result\`1<!!T> to a Result\`1<object> param (Result is an invariant REFERENCE class, so no covariance). bir2cir must either make throwOnFailure generic (throwOnFailure<T>(Result<T>)) or insert the erasure at the star-projection boundary. NOT ilemit (the arg/param types come from the CIR as-is)."
	...
	[tryexprop]="bundle-6 edge: a value-position try/catch(/finally) used in an OPERAND slot (\`1 + try{..}\`, \`\"x\" + try{..}\`) -> InvalidProgramException. kotc emits the CORRECT value-form (valueBlock = [var tmp; try{setLocal tmp}catch{setLocal tmp}], result=local(tmp)) — verified layer-pure; the manually-hoisted \`val t = try{..}; a + t\` form RUNS correct. Root cause is downstream: a CLR protected region must be entered with an EMPTY eval stack (\`leave\` clears it), but ilemit's valueBlock runs the try INLINE after the left operand was pushed, wiping it. Fix = bir2cir CLR eval-order normalization: hoist the try-bearing valueBlock out of the operand to a preceding temp (preserving left-to-right eval order). Not kotc (no CLR-stack knowledge), not ilemit (no Kotlin-aware rescheduling)."
)
declare -A XFAIL_ILVERIFY=(
	[iter]="bir2cir/kotc DUAL-REPRESENTATION defect (NOT ilemit): the BIR carries TWO distinct CLR identities for the one Kotlin type Iterator<Int> — the app-local monomorphized synthetic '<>dotkt_KIterator_kotlin_Int' AND 'clrg:kotlin.collections.Iterator[int]' (the rt stdlib generic). ilemit faithfully emits both as distinct CLR types, so a return/store typed as one where the other is expected is StackUnexpected (found kotlin.collections.Iterator\`1<int32>, expected <>dotkt_KIterator_kotlin_Int). RUNS correct (interface refs are object refs; the mismatch only bites at actual dispatch, which always hits the concrete impl). Codex-confirmed layer boundary: unifying Iterator<Int> to ONE canonical CLR representation is a Kotlin<->CLR type-lowering decision (bir2cir/kotc — retire the '<>dotkt_KIterator_*' synthetic in favor of the rt generic, or vice versa); an ilemit castclass would be a hacky layer violation (the value would still carry two CLR identities)."
	[iterable]="same bir2cir/kotc dual-representation defect as iter: '<>dotkt_KIterator_kotlin_Int' (app synthetic) vs 'clrg:kotlin.collections.Iterator[int]' (rt stdlib generic) for one Kotlin Iterator<Int>. Also surfaces in the ilemit-emitted covariant bridge Range3::<>dotkt_covar\$iterator\$0 (it forwards the body's rt-typed Iterator to the synthetic-typed slot). RUNS 321/6/6 correct. Fix = bir2cir canonical Iterator representation (not ilemit)."
	[tryexprop]="ilverify counterpart of run:tryexprop — the operand-position value-try emits an invalid protected region (try entered with a non-empty eval stack). Pends the bir2cir hoist-out-of-operand normalization."
)
...
# digittoint: Char.digitToIntOrNull()/digitToInt() value-type-nullable (Int?) return -> 7/10/null/7. Currently
# run-XFAIL: kotc mis-types the inlined takeIf if-expression cond (see XFAIL_RUN[digittoint]); NOT an ilemit bug.
