...
# CLR-SPECIFIC-BY-DESIGN (would DIFF for a documented reason, not a bug): il-fmt (String.format
...
il-seq il-sort il-getclass il-localdeleg il-langfeat il-localclass il-refcell il-props \
il-deleg il-deleg2 il-dsl il-duration il-exprbody il-ext il-for il-groupvalues il-iface il-inner il-langtail il-lazy il-loopjump il-nested il-overload il-overrideprop il-pair il-regex il-regexgroups il-regexreplace il-rwp il-samcmp il-seqfilter il-setlocalbox il-unsigned il-use il-valclass il-vis il-volatile il-xfaceimpl il-xprop \
il-boxgen il-pairtostr il-extprop il-defargs il-defargs2 il-pairnest \
il-genmax il-nestlam il-genseq2 il-cwindowedv \
il-triple il-typealias il-atomics il-tailrec il-equalscall"
...
# il-equalscall (§5a): explicit .equals() routing.
