...
il-genmax il-listplus il-divmin il-nestlam il-genseq2 il-cwindowed il-cwindowedv il-eachcount il-groupby2 \
il-triple il-typealias il-atomics"
...
# NOT in PURE: il-tailrec — deep `tailrec` (1e6 frames) STACK-OVERFLOWS on the CLR (no TCO emitted); the JVM
# oracle TCOs it and prints the sum, so it is a genuine DIFF/crash, documented in docs/dotkt-semantics.md
# (kotc/bir2cir tail-call-lowering gap) and left OUT rather than XFAIL'd (the gate stays XFAIL-zero).
