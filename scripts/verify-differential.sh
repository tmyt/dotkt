...
# uses .NET COMPOSITE format strings `{0:F2}`/`{0:D5}` — literal text on the JVM, not printf).
#
...
il-seq il-char il-sort il-getclass il-localdeleg il-langfeat il-ctorref il-localclass il-annot \
il-bymap il-bytearg il-chunk il-cmpord il-comparable il-comparator il-cp il-ctor il-deleg il-deleg2 il-dsl il-duration il-exprbody il-ext il-for il-groupvalues il-iface il-inner il-langtail il-lazy il-loopjump il-regex il-regexgroups il-regexreplace il-samcmp il-seqfilter il-setlocalbox il-unsigned il-use il-valclass il-vis il-volatile il-xfaceimpl il-xprop \
il-boxgen il-pairtostr il-extprop il-defargs il-defargs2 \
il-genmax il-genseq2 il-cwindowed il-cwindowedv \
