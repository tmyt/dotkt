...
il-char il-getclass il-localdeleg il-langfeat il-ctorref il-localclass il-refcell il-annot il-props \
...
il-atomics il-copydef il-equalscall"
# COV2 (kcc review §2B, 2026-07-06): il-atomics (kotlin.concurrent.atomics — the @ClrRefArgument
...
# MIGRATED OUT (batch M5, into the in-process NUnit il-battery — value asserts subsume the JVM oracle, design D3):
# il-seq/il-sort/il-seqfilter/il-setlocalbox/il-unsigned/il-use/il-valclass/il-vis/il-volatile/il-xfaceimpl/il-xprop
# and il-triple/il-typealias/il-tailrec (COV2/COV3/COV4) — removed from PURE in the same change as their case dirs.
