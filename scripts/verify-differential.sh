...
il-arr il-bymap il-bytearg il-chunk il-closure il-cmpord il-coerce il-comparable il-comparator il-cp il-ctor il-customexc il-deleg il-deleg2 il-dsl il-duration il-enum il-enumbody il-enumrich il-exc il-excmap il-exprbody il-ext il-for il-genbase il-genclosure il-gencolladd il-genctor il-genhof il-genstatic il-gfac il-groupvalues il-iface il-inner il-lambda il-langtail il-lazy il-loopjump il-math il-mfclosure il-mflambda il-nested il-nestedtry il-object il-objexpr il-objgen il-op il-ops il-overload il-overrideprop il-pair il-rangein il-regex il-regexgroups il-regexreplace il-result il-rwp il-samcmp il-scope il-seqfilter il-setlocalbox il-smartcast il-throwexpr il-tryexprop il-unsigned il-use il-userrange il-valclass il-vis il-volatile il-whensubj il-xfaceimpl il-xprop \
il-boxgen il-mathabs il-pairtostr il-extprop il-defargs il-defargs2 il-indices il-pairnest \
