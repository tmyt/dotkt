...
DotKt reuses the stock Kotlin frontend and replaces the backend with a four-stage CLR pipeline:

```text
Kotlin source -> kotc -> BIR -> bir2cir -> CIR -> ilemit -> CIL
                     ^                  ^
              facadegen metadata   CLR reference metadata
```
...
|---|---|---|---|
| `facadegen` | Referenced CLR assemblies | CLR metadata to Kotlin frontend-injection metadata | Kotlin stdlib bindings |
| `kotc` | Kotlin source, stdlib frontend KLIB, facadegen metadata | PSI/FIR/IR processing and BIR serialization | CLR/BCL policy or `@Clr*` interpretation |
...
1. `kotlin.*` symbols come from the frontend KLIB, never from facadegen.
