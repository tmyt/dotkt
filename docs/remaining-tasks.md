...
- コルーチン `suspend`（直線/ループ/分岐 await・直接 suspend 呼出・**部分式内サスペンドの spilling** `f(a.await())`/`a.await()+b.await()`・**条件式内サスペンド** `while(f().await())`/`if(g().await())`・**try/catch-around-await** `try{ a.await() }catch(e){ … }`＝言語レベルはほぼ網羅、残＝finally-around-await・catch 内 suspend のみクリーンエラー）（`il-coro`）
...
- [~] **可視性修飾子** `private`/`internal`/`protected`/`public` → IL アクセス修飾子。**(S)** — **メソッド/コンストラクタ/クラス（トップレベル）に実装**: `visOf`（Kotlin `Visibilities`→`public`/`private`/`internal`/`protected`）→ ilemit `AccessOf`（`MethodAttributes.Private`/`Assembly`/`Family`/`Public`、型は `TypeAttributes.Public`/`NotPublic`）。interface メンバ・Object-override・匿名/合成型は public 固定。reflection で flag 検証済（private/asm/family 正）。`samples/il-vis`。**残**: **フィールド可視性**（現状 property=public field モデルゆえ public 固定。真の private/getter-setter は property モデル導入時）。protected override の可視性整合も将来。
- [~] **enum リッチ API**: メンバ/メソッド/抽象メンバを持つ enum。**(M-L)** — **基本 enum は CLR enum 維持（.NET interop）、rich enum は singleton class へ二経路化（Codex 確認）＝実装完了**。`isRichEnum`（ctor 引数 or ユーザメソッド or エントリ本体）→ `richEnumDef`: 平坦 class（`__name`/`__ordinal`+ユーザ field、private ctor、エントリは `static readonly` field を `.cctor` で `new`、`ToString`→`__name`、`values()`→fresh array、`valueOf`→線形 match＋`ArgumentException`、`==`は singleton 参照同一性）。消費側ルーティング（`IrGetEnumValue`→staticField、`.name`/`.ordinal`→field、`values`/`valueOf`→callStatic）。`samples/il-enumrich`（mass/heavy/name/ordinal/valueOf/values-for/== 実機正＋ilverify）。**残**: **per-entry 本体/抽象メンバ**（`PLUS { override fun apply… }`＝各エントリを匿名サブクラスに、base を abstract 非 sealed に）— Codex 確認の shape を 2nd step で。
...
| per-entry 本体/抽象メンバ enum（`X { override … }`） | A-2 enum rich | 基本/ctor引数/メソッド enum は済 | enum 2nd step（匿名サブクラス＝Codex 確認の shape 適用） |
| フィールド可視性（真の private フィールド） | A-2 可視性 | property-as-field＝public 固定 | 真の property モデル（getter/setter）導入時 |
...
- [x] コレクション操作 → LINQ 静的写像。**済 30+種**: （`m-b1`,`m-b3`,`m-b6`,`m-b9`、値返しラムダ）。残: `groupBy`/`associate`/`zip`/`mapIndexed`/`windowed`/`chunked`（一部は Map 返し/index 順差で要個別対応）。**(M)** ✅（主要）
- [~] `Sequence`（遅延）/ `asSequence` / `generateSequence`。**(M)** — ✅ 2026-06-20 **`asSequence` ＋遅延シーケンス操作**: `Sequence<T>`→**遅延 .NET `IEnumerable<T>`**（LINQ は元々 deferred なので Kotlin の遅延意味論と一致）。`isSequenceType` を導入、collection-ops ブロックのゲートを `isCollectionType || isSequenceType` に拡張、`lazySeq` フラグで中間 list 生成op（map/filter/take/…）の `ToList` материализ化を抑止（eager コレクションは従来通り ToList、シーケンスは deferred 維持）。`asSequence()`→受け手の pass-through、`toSet`→`ToHashSet`／`takeWhile`/`dropWhile`（→`TakeWhile`/`SkipWhile`、遅延）／`single`/`singleOrNull`（→`Single`/`SingleOrDefault`）も追加。終端（toList/first/count/sum/single）が初めて評価を強制。`samples/il-seq`（map→filter→toList=`6,12`／map→filter→first 短絡=16／filter→count=3／map→sum=27／map→take→toList=`10-20-30`）実機正＋ilverify-clean＋**JVM 差分一致**（PURE corpus に追加）。残: `generateSequence`/`sequence{}` ビルダー（遅延ジェネレータ＝iterator 合成が必要、別増分）。
...
- [~] `Pair`/`Triple`/`to` → C# ValueTuple（`.first`→Item1 等）。`m-b5`。残: `Result`、`Comparable`/`Comparator`。**(M)**
