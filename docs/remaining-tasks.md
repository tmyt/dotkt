...
- [x] コレクション操作 → LINQ 静的写像。**済 30+種**: （`m-b1`,`m-b3`,`m-b6`,`m-b9`、値返しラムダ）。**mapIndexed 済**（2026-06-20、`il-bmore`）= `Range(0,MAX).Zip(src, Func<int,T,R>)`（Zip の (first,second)=(index,value) が Kotlin 順と一致＝引数 swap 不要）。残: `groupBy`/`associate`/`zip`（一部は Map 返し）、**`chunked`**（.NET `Chunk` は `T[]` を返すが Kotlin は `List<T>` 型付け＝要 T[]→List 変換の合成ラムダ）、`windowed`、**`sortedWith(compareBy{})`**（compareBy は inline で selector が `Comparable<*>` に erase され実キー型が取れない＝sortedBy/sortedByDescending で単一キーは対応済）。**(M)** ✅（主要）
