...
- an `address` binding never becomes a value-copy `var` — no storage holds a managed pointer — and splits by what its location's
  ...
  setter/call to a temporary struct copy instead of the source location. Value-type links inside the surviving
  location likewise remain addressable links: ilemit consumes the CIR path recursively as `ldelema`/`ldflda`, while
  a reference-type owner is loaded as a value. A real property accessor remains an rvalue and therefore uses the
  ordinary temporary-address fallback; only a CIR access explicitly resolved as a field denotes field storage;
...
keeping (`ret` would answer the same, so nothing is gained by dropping), while a physical erasure that genuinely
changes the result must not leave a semantic stamp that downstream trusts first. For constructed member returns,
