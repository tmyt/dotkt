...
- #4 skipStdlibHighArityFunctionType :584 (uses 406/539/672/700/736/1113/1118) — MOVED-TO-bir2cir, then RETIRED (#56, 2026-07-20). The interim `HighArityFunctionFilter` still dropped >16-param-function-type decls in stdlib mode (a silent-drop landmine; its "6 context() overloads arity 17-22" premise was false — those overloads are arity 1-6). Retired entirely: ilemit's KFunc`N/KAction`N synthesis (Emitter.Delegates.cs/Emitter.Types.cs) is arity-driven and mode-INDEPENDENT, so the stdlib build now emits wide function-type decls exactly like an app build (empirically: ref+rt emit KFunc`23/KAction`22). The Func/Action 16-cap is a CLR-rep fact owned solely by ilemit's BuildFuncType; no bir2cir arity filter remains.
...
4. #4: deleted kotc's :584 + 7 filterNot sites. (The interim bir2cir stdlib-mode Fn-arity decl filter was later retired entirely — #56; ilemit KFunc/KAction synthesis covers all modes, so no arity filter remains.)
