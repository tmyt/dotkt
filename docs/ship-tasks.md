...
>
> **RECONCILE 2026-07-08 (#52 kotc-purity):** kotc now recognizes ZERO operators, reads no `@Clr*`, and is substitute-independent (ref/rt BIR bit-identical, #66) — the stdlib-recognition axis of §0's kotc responsibility is fully realized (bir2cir owns all substitution). NB the "A2 interop-no-registry (4 registries deleted)" above is a DIFFERENT, partial item; the **facadegen-interop A2 = task #61 has since LANDED** — kotc emits the plain `callStatic`/`callInstance` by FQN identity and bir2cir's `NetInteropBinding` reflects the owner against the .NET reference assemblies to bind the `clrStatic`/`clrInstance`/`clrPropGet`/… shape (CLR-only vocabulary — events, byref — stays kotc-lowered by design). Remaining kotc-purity = naming purity (`generated:true`/CharSequence). See `docs/master-task-inventory.md` 【6b】 + `docs/kotc-recognition-audit.md`.
