...
2. **Wave 2 (next):** composite physical primitives — `spreadConcat`, `stackAsSpan`, `constrainedCall` (`newArray` already done in Wave 1). Then **Milestone 0**: flip the default to `--native-cir` and **delete `--compat-bir`**, gated only on `verify-native-cir-ilemit.sh` + the full corpus going green; that flip is what lets the legacy ilemit cases and intrinsics be deleted.
