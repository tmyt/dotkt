...
| [2](#2-generics-are-reified--nullable-sensitive-uses-carry-a-separate-witness) | Reified generics — CLR runtime type plus structurally demanded Kotlin nullability witness |
...
## 2. Generics are reified — nullable-sensitive uses carry a separate witness
...
- **Generic members retain owner-dependent Kotlin constraints as metadata, not CLR constraint rows.**
  ...
  Suspend members use the same physical constraints on their Task and cold-entry slots. Their state machines
  concatenate the enclosing owner's and the method's generic frames, preserving both across suspension and capture.
  The Task projection retains the Kotlin constraint and source-member metadata used by separately compiled consumers.
...
- **Corollary — a star-projected collection classifier must erase its element arguments.** On the JVM `x is Map<*,*>` and a subsequent
  ...
  `Map<*,*>` retains its non-generic `IDictionary` path. `Collection<*>`, `MutableCollection<*>`, `List<*>`, `MutableList<*>`,
  ...
  the receiver. An existing generic Count view retains precedence; raw Count is used only when no generic
  Collection face exists, not to mask an ambiguous view or a failing getter. Pinned by `RawCollectionIsEmptyTests`,
...
- **Iterable identity is distinct from its CLR enumeration face.** Kotlin implementations carry nominal
  ...
  Strings, arrays, and dictionary storage do not acquire Kotlin `Iterable` identity merely by exposing CLR
  enumeration. A class that explicitly implements Kotlin `Iterable` retains that declaration-owned identity,
...
- Consequences:
  - **`inline` is decoration UNLESS the call passes a lambda LITERAL.** A lambda-less `inline fun`
    ...
    Kotlin `reified` remains a source declaration fact; nullable-sensitive operations separately control the witness
    ABI described in §2.
...
| generics, `reified` | `[KotlinDeclarationIdentity]` separately records semantic reified method-type-parameter indices and physical nullable-witness indices (§2) |
...
| `EmitSuspensionPoint` resume slot; `IsUnitTn(retTok) -> AnyTn` | LEGITIMATE-ABI | The cold entry's physical result slot is `Any?`. Successful Unit completion carries the Kotlin `Unit` singleton, not null; the object-valued slot preserves that value across direct completion and resumption. |
...
- `inline` written but no lambda passed → ordinary call; nullable-sensitive generic operations still receive their witness. §2, §3.
- `reified` keeps Kotlin's ordinary source restrictions even though the CLR carries runtime generic types. §2.
