...
static class InlineSplice
{
    ...
    sealed record BoxRequest(string VarName, string RefName, JsonNode CellType, JsonNode Elem, bool Synthesize,
    ...
    static Dictionary<string, JsonNode> PropagateSplicedCaptures(JsonObject lam, JsonObject host)
    {
        ...
        foreach (var c in lamCaps.OfType<JsonObject>())
        {
            ...
            if (existing != null)
            {
                ...
                if (c["sharedCellType"] is JsonNode shared)
                {
                    if (existing["sharedCellType"] is JsonNode previous && !JsonNode.DeepEquals(previous, shared))
                        throw new InvalidOperationException($"inline capture '{cn}' has conflicting shared locations");
                    existing["sharedCellType"] = shared.DeepClone();
                    existing["sharedCellTypeParams"] = c["sharedCellTypeParams"]?.DeepClone();
                }
            ...
            if (!outer)
            {
                ...
                if (hostScope.Contains(cn))
                {
                    ...
                    var forwarded = (JsonObject)c.DeepClone();
                    forwarded["name"] = fresh;
                    forwarded["value"] = srcVal;
                    hostCaps.Add(forwarded);
        ...
        JsonObject carrier, JsonNode funcType, JsonArray stmts, string semanticOwner)
    {
        ...
        var declaredCells = new Dictionary<string, JsonNode>(StringComparer.Ordinal);
        var declaredCaptures = new Dictionary<string, JsonObject>(StringComparer.Ordinal);
        if (carrier["captures"] is JsonArray mutableCaptures)
            foreach (var capture in mutableCaptures.OfType<JsonObject>())
                if (Str(capture["name"]) is string capturedName && capNames.Contains(capturedName)
                    && capture["sharedCellType"] is JsonNode cellType)
                {
                    declaredCells.Add(capturedName, cellType);
                    declaredCaptures.Add(capturedName, capture);
                    boxedCaps.Add(capturedName);
                }
        ...
        if (boxedCaps.Count > 0)
            foreach (var f in fields.OfType<JsonObject>())
                if (Str(f["name"]) is string fn && boxedCaps.Contains(fn) && !declaredCells.ContainsKey(fn)
                    && HasTv(f["type"])) return MatNull("MC:boxed-cap-has-tv");
        ...
        if (boxedCaps.Count > 0)
        {
            var boxRefName = new Dictionary<string, JsonNode>(StringComparer.Ordinal);
            ...
            foreach (var f in fields.OfType<JsonObject>())
            {
                ...
                var supplied = declaredCells.TryGetValue(fn, out var declaredCell);
                var cellType = supplied ? declaredCell.DeepClone() : TypeJson.Fqn(RefCellNameFor(elem));
                boxRefName[fn] = cellType;
                if (elem != null) boxRefElem[fn] = elem.DeepClone();
                f["type"] = cellType.DeepClone();
                RequestBox(supplied ? declaredCaptures[fn] : null, fn, cellType, elem, !supplied);
            }
        ...
        foreach (var key in keys)
        {
            ...
            typeParams.Add(captureConstraints.TryGetValue(key, out var bounds)
                ? new JsonObject { ["name"] = "T" + ni, ["constraints"] = bounds.DeepClone() }
                : JsonValue.Create("T" + ni));
        }
        if (remap.Count > 0)
        {
            ...
            RenumberTvs(typeParams, remap, classFrame: true);
        }
    ...
    static string MaterializeSuspendCarrier(JsonObject carrier, JsonObject ft, JsonArray lamParams, JsonArray lamBody, JsonNode lamResult, JsonArray stmts)
    {
        ...
        // Reconcile every capture with this generated state-machine namespace while preserving the explicit outer role.
        // A capture with no name/type means kotc failed to provide the declaration fact -> refuse.
        ...
        if (carrier["captures"] is JsonArray caps)
            foreach (var c in caps.OfType<JsonObject>())
            {
                ...
                // nested inline carriers. The positional capValues channel states the original construction value for
                // every renamed/outer slot; no downstream pass reconstructs that edge from descriptor spelling.
                var storageName = outer
                    ? FreshCaptureStorageName("cap$__outer")
                    : captureStorageNames.Add(cn) ? cn : FreshCaptureStorageName("cap$" + cn);
                // Ordinary nested descriptors are name-keyed. An outer descriptor is role-keyed instead, and must not
                // make a distinct ordinary nested capture with the same spelling alias the enclosing receiver.
                if (!outer) sourceStorageNames.Add(cn, storageName);
                var descriptor = new JsonObject { ["name"] = storageName, ["type"] = ct.DeepClone() };
                if (!outer && c["sharedCellType"] is JsonNode cellType)
                {
                    descriptor["type"] = cellType.DeepClone();
                    BoxVarInBody(invBody, cn, cellType, ct);
                    RequestBox(c, storageName, cellType, ct, false);
                }
                if (outer) descriptor["outer"] = true;
                captures.Add(descriptor);
                if (!outer && storageName != cn)
                {
                    captureRenames.Add(cn, new JsonObject { ["k"] = "local", ["name"] = storageName });
                }
                if (c["value"] is JsonNode explicitValue)
                {
                    suspendCapValues.Add(explicitValue.DeepClone());
                    anySuspendCapValue = true;
                }
                else if (outer || storageName != cn)
                {
                    suspendCapValues.Add(outer
                        ? new JsonObject { ["k"] = "this" }
                        : new JsonObject { ["k"] = "local", ["name"] = cn });
                    anySuspendCapValue = true;
                }
                else suspendCapValues.Add(null);
            }
        if (captureRenames.Count > 0) RewriteLocalRefs(invBody, captureRenames);
        ...
        foreach (var nested in CurrentFrameNestedSuspendLambdas(invBody))
        {
            ...
            void SetNestedValue(int index, string storageName)
            {
                ...
                {
                    nestedValues = new JsonArray();
                    while (nestedValues.Count < nestedCaps.Count) nestedValues.Add(null);
                    nested["capValues"] = nestedValues;
                }
                nestedValues[index] = new JsonObject { ["k"] = "local", ["name"] = storageName };
            }
            for (int i = 0; i < nestedCaps.Count; i++)
            {
                ...
                if (Bool(nc["outer"]))
                {
                    var existingOuter = captures.OfType<JsonObject>().FirstOrDefault(c => Bool(c["outer"]));
                    if (existingOuter != null)
                    {
                        if (!JsonNode.DeepEquals(existingOuter["type"], nt))
                            return MatNull("MSC:nested-outer-type-conflict");
                        SetNestedValue(i, Str(existingOuter["name"]));
                        continue;
                    }
                    var outerStorageName = FreshCaptureStorageName("cap$__outer");
                    captures.Add(new JsonObject {
                        ["name"] = outerStorageName,
                        ["type"] = nt.DeepClone(),
                        ["outer"] = true,
                    });
                    suspendCapValues.Add(new JsonObject { ["k"] = "this" });
                    anySuspendCapValue = true;
                    SetNestedValue(i, outerStorageName);
                    continue;
                }
                if (innerScope.Contains(nn)) continue;

                var existingStorageName = sourceStorageNames.TryGetValue(nn, out var sourceStorage)
                    ? sourceStorage : nn;
                var existing = captures.OfType<JsonObject>().FirstOrDefault(c => Str(c["name"]) == existingStorageName);
                if (existing != null)
                {
                    if (!JsonNode.DeepEquals(existing["type"], nt)) return MatNull("MSC:nested-capture-type-conflict");
                    if (existingStorageName != nn) SetNestedValue(i, existingStorageName);
                    continue;
                }
                var promotedStorageName = captureStorageNames.Add(nn) ? nn : FreshCaptureStorageName("cap$" + nn);
                sourceStorageNames[nn] = promotedStorageName;
                captures.Add(new JsonObject { ["name"] = promotedStorageName, ["type"] = nt.DeepClone() });
                if (promotedStorageName == nn) suspendCapValues.Add(null);
                else
                {
                    suspendCapValues.Add(new JsonObject { ["k"] = "local", ["name"] = nn });
                    anySuspendCapValue = true;
                    SetNestedValue(i, promotedStorageName);
                }
            }
        }
        ...
        var typeParams = new JsonArray();
        var typeParamDecls = new JsonArray();
        foreach (var key in keys)
        {
            int ni = remap.Count;
            remap[key] = ni;
            ...
            typeParamDecls.Add(captureConstraints.TryGetValue(key, out var bounds)
                ? new JsonObject { ["name"] = "Tsm" + ni, ["constraints"] = bounds.DeepClone() }
                : JsonValue.Create("Tsm" + ni));
        }
        ...
        if (remap.Count > 0)
        {
            ...
            RenumberTvs(typeParamDecls, remap);
        }
    ...
    static Dictionary<(string scope, int i), JsonArray> SharedCaptureConstraints(JsonObject carrier)
    {
        ...
        if (carrier["captures"] is not JsonArray captures) return result;
        foreach (var capture in captures.OfType<JsonObject>())
        {
            if (capture["sharedCellType"] is not JsonObject cell || cell["args"] is not JsonArray args) continue;
            if (capture["sharedCellTypeParams"] is not JsonArray declarations || declarations.Count != args.Count)
                throw new InvalidOperationException("shared capture is missing its cell's generic declarations");
            for (int i = 0; i < args.Count; i++)
            {
                if (TypeJson.Read(args[i]) is not TypeNode.Tv tv
                    || declarations[i] is not JsonObject declaration || declaration["constraints"] is not JsonArray bounds)
                    continue;
                var instantiated = (JsonArray)bounds.DeepClone();
                SubstTvIn(instantiated, new JsonArray(), 0, args);
                var key = (tv.Scope, tv.I);
                if (!result.TryGetValue(key, out var combined)) result[key] = combined = new JsonArray();
                foreach (var bound in instantiated)
                    if (!combined.Any(existing => JsonNode.DeepEquals(existing, bound))) combined.Add(bound.DeepClone());
            }
        }
        return result;
    }
    ...
    static bool HasTvType(TypeNode t) => t switch
    {
        ...
        TypeNode.Projection p => HasTvType(p.Of),
        TypeNode.Array a => HasTvType(a.Elem),
        TypeNode.ByRef b => HasTvType(b.Of),
        TypeNode.Ptr p => HasTvType(p.Of),
        TypeNode.Mod m => HasTvType(m.M) || HasTvType(m.Of),
    ...
    static void RewriteBoxedCaptureInCarrier(JsonNode node, HashSet<string> boxed, Dictionary<string, JsonNode> refName, Dictionary<string, JsonNode> refElem, string cname)
    {
    ...
    static void RewriteBoxedWrite(JsonObject o, string x, Dictionary<string, JsonNode> refName, string ownerCname)
    {
        ...
        o["k"] = "setField";
        o["ownerType"] = refName[x].DeepClone();
        o["recv"] = recv;
        o["name"] = "v";
        o["value"] = value;
    }
    ...
    // that OWNS the cell (see MaterializedCaptureSources) — the decl `var X = init` -> `var X:Ref = new Ref(init)`, every read
    ...
    static void RequestBox(JsonObject capture, string captureName, JsonNode cellType, JsonNode elem, bool synthesize)
    {
        var sourceName = capture == null ? captureName : Str(capture["name"]);
        if (capture?["value"] is JsonObject value)
            sourceName = Str(value["k"]) == "local" ? Str(value["name"])
                : throw new InvalidOperationException("a mutable inline capture must retain its source local location");
        var refName = TypeJson.Read(cellType) is TypeNode.Fqn cell ? cell.Name
            : throw new InvalidOperationException("inline capture sharedCellType must name its cell declaration");
        var existing = _boxRequests.FirstOrDefault(r => r.VarName == sourceName && JsonNode.DeepEquals(r.CellType, cellType));
        if (existing != null) existing.CaptureNames.Add(captureName);
        else _boxRequests.Add(new BoxRequest(sourceName, refName, cellType.DeepClone(), elem.DeepClone(), synthesize,
            new HashSet<string>(StringComparer.Ordinal) { captureName }));
    }
    ...
    static void BoxMaterializedCaptures(JsonNode root)
    {
        ...
        foreach (var request in _boxRequests)
            if (request.Synthesize && present.Add(request.RefName))
                reg.Add(new JsonObject { ["name"] = request.RefName, ["elem"] = request.Elem.DeepClone(), ["typeParams"] = new JsonArray() });
        ...
        // var in an unrelated method — including one kotc ALREADY ref-cell-boxed — is never touched (no double-boxing).
        // A local-function capture parameter instead forwards the requirement to its callers. Runs BEFORE
        // ClosureSynthesis strips `synthClass`, so fields and their construction arguments are still visible here.
        var scopes = new List<(JsonObject Declaration, JsonNode Body, string LocalId)>();
        void CollectLocals(JsonNode node)
        {
            if (node is JsonObject obj)
            {
                if (Str(obj["k"]) == "localFun" && obj["decl"] is JsonObject decl && decl["body"] is JsonArray body)
                {
                    scopes.Add((decl, body, Str(obj["id"])));
                    CollectLocals(body);
                    return;
                }
                if (obj["synthClass"] is JsonObject synth && synth["body"] is JsonArray closureBody)
                {
                    scopes.Add((synth, closureBody, null));
                    CollectLocals(closureBody);
                }
                bool suspend = Str(obj["k"]) == "newSuspendLambda";
                if (suspend && obj["body"] is JsonArray suspendBody)
                {
                    scopes.Add((obj, suspendBody, null));
                    CollectLocals(suspendBody);
                }
                foreach (var pair in obj)
                    if (pair.Key != "synthClass" && !(suspend && pair.Key == "body")) CollectLocals(pair.Value);
            }
            else if (node is JsonArray array) foreach (var child in array) CollectLocals(child);
        }
        foreach (var body in AllBodies(file))
        {
            scopes.Add(((JsonObject)body.Parent, body, null));
            CollectLocals(body);
        }
        void CollectInitializers(JsonNode fields)
        {
            if (fields is not JsonArray array) return;
            foreach (var field in array.OfType<JsonObject>())
                if (field["init"] is JsonNode initializer)
                {
                    scopes.Add((field, initializer, null));
                    CollectLocals(initializer);
                }
        }
        CollectInitializers(file["fields"]);
        if (file["types"] is JsonArray types)
            foreach (var type in types.OfType<JsonObject>()) CollectInitializers(type["fields"]);
        var pending = new Queue<(int Scope, BoxRequest Request)>();
        for (int i = 0; i < scopes.Count; i++)
            foreach (var request in _boxRequests)
                foreach (var source in MaterializedCaptureSources(scopes[i].Body, request))
                    pending.Enqueue((i, request with { VarName = source }));
        var completed = new HashSet<(int Scope, string Name, string Cell)>();
        var rewrites = new List<(int Scope, BoxRequest Request)>();
        while (pending.TryDequeue(out var entry))
        {
            var request = entry.Request;
            var scope = scopes[entry.Scope];
            if (!completed.Add((entry.Scope, request.VarName, request.CellType.ToJsonString()))) continue;
            if (scope.Declaration["params"] is JsonArray parameters)
            {
                int index = Enumerable.Range(0, parameters.Count).FirstOrDefault(
                    i => Str(parameters[i]?["name"]) == request.VarName, -1);
                if (index >= 0)
                {
                    if (scope.LocalId == null || parameters[index]?["sharedCellType"] == null)
                        throw new InvalidOperationException("shared inline capture is missing its lexical parameter location");
                    parameters[index]["type"] = request.CellType.DeepClone();
                    // A local function's capture parameter is a forwarding edge, not a new storage location.
                    // Update every call by declaration identity, then propagate the demand into each caller's frame.
                    for (int caller = 0; caller < scopes.Count; caller++)
                    {
                        void VisitCalls(JsonNode node)
                        {
                            if (node is JsonObject call)
                            {
                                if (Str(call["k"]) == "localFun") return;
                                if (Str(call["k"]) == "callLocal" && Str(call["id"]) == scope.LocalId)
                                {
                                    if (call["args"] is not JsonArray args || args[index] is not JsonObject arg)
                                        throw new InvalidOperationException("shared local-function capture must pass its source location");
                                    var cell = request.CellType.DeepClone();
                                    var elem = request.Elem.DeepClone();
                                    var typeArgs = call["typeArgs"] as JsonArray ?? new JsonArray();
                                    SubstTvIn(cell, typeArgs, typeArgs.Count);
                                    SubstTvIn(elem, typeArgs, typeArgs.Count);
                                    ((JsonArray)call["sig"])[index] = cell.DeepClone();
                                    // Materialization already redirected a captured read through this cell's value.
                                    // The local-function capture argument must forward that existing cell, not its value.
                                    if (Str(arg["k"]) == "field" && Str(arg["name"]) == "v"
                                        && JsonNode.DeepEquals(arg["ownerType"], cell) && arg["recv"] is JsonNode cellValue)
                                    {
                                        args[index] = cellValue.DeepClone();
                                    }
                                    else
                                    {
                                        if (Str(arg["k"]) != "local" || Str(arg["name"]) is not string source)
                                            throw new InvalidOperationException("shared local-function capture must pass its source location");
                                        if (!_boxedLocalCallArgs.TryGetValue(call, out var slots))
                                            _boxedLocalCallArgs[call] = slots = new();
                                        slots.Add(index);
                                        pending.Enqueue((caller, new BoxRequest(source, request.RefName, cell, elem, false, new())));
                                    }
                                }
                                foreach (var pair in call)
                                    if (pair.Key != "synthClass" && !(Str(call["k"]) == "newSuspendLambda" && pair.Key == "body"))
                                        VisitCalls(pair.Value);
                            }
                            else if (node is JsonArray array) foreach (var child in array) VisitCalls(child);
                        }
                        VisitCalls(scopes[caller].Body);
                    }
                }
            }
            rewrites.Add(entry);
        }
        // Resolve the whole forwarding graph before changing reads: a caller may feed several local functions.
        foreach (var entry in rewrites)
        {
            var request = entry.Request;
            var scope = scopes[entry.Scope];
            if (BoxUnsafeReason(scope.Body, request.VarName, request.RefName, request.Elem) is string reason)
                throw new NotSupportedException($"inline splice: cannot ref-cell-box captured var '{request.VarName}' — {reason}");
            BoxVarInBody(scope.Body, request.VarName, request.CellType, request.Elem);
        }
    }
    ...
    static string BoxUnsafeReason(JsonNode body, string x, string refName, JsonNode elem)
    {
        ...
        void Rec(JsonNode n)
        {
            ...
            if (n is JsonObject o)
            {
                ...
                foreach (var kv in o)
                    if (kv.Key != "synthClass" && !(Str(o["k"]) == "newSuspendLambda" && kv.Key == "body")) Rec(kv.Value);
            }
    ...
    // Constructor arguments, not the names recorded before inline alpha-renaming, identify the source location.
    static IEnumerable<string> MaterializedCaptureSources(JsonNode node, BoxRequest request)
    {
        if (node is JsonObject o)
        {
            if (Str(o["k"]) == "localFun") yield break;
            bool suspend = Str(o["k"]) == "newSuspendLambda";
            var fields = suspend ? o["captures"] as JsonArray : o["synthClass"]?["fields"] as JsonArray;
            var values = suspend ? o["capValues"] as JsonArray : o["captures"] as JsonArray;
            if (fields != null)
                for (int i = 0; i < fields.Count; i++)
                {
                    if (fields[i] is not JsonObject field || Str(field["name"]) is not string name
                        || !request.CaptureNames.Contains(name)
                        || TypeJson.Read(field["type"]) is not TypeNode.Fqn type || type.Name != request.RefName)
                        continue;
                    var value = values?.ElementAtOrDefault(i);
                    if (value == null && suspend) yield return name;
                    else if (value is JsonObject local && Str(local["k"]) == "local" && Str(local["name"]) is string source)
                        yield return source;
                }
            foreach (var kv in o)
                if (kv.Key != "synthClass" && !(suspend && kv.Key == "body") && kv.Value != null)
                    foreach (var source in MaterializedCaptureSources(kv.Value, request)) yield return source;
        }
        else if (node is JsonArray array)
            foreach (var child in array)
                if (child != null)
                    foreach (var source in MaterializedCaptureSources(child, request)) yield return source;
    }
    ...
    static void BoxVarInBody(JsonNode node, string x, JsonNode refName, JsonNode elem)
    {
        ...
        {
            ["k"] = "field",
            ["ownerType"] = refName.DeepClone(),
            ["recv"] = new JsonObject { ["k"] = "local", ["name"] = x },
            ["name"] = "v",
            ["sty"] = elem?.DeepClone(),
        };
        var refMap = new Dictionary<string, JsonNode>(StringComparer.Ordinal) { [x] = refName };
        ...
        if (Str(o["k"]) == "var" && Str(o["name"]) == x)
        {
            ...
            o["type"] = refName.DeepClone();
            o["init"] = new JsonObject
            {
                ["k"] = "new", ["type"] = refName.DeepClone(),
        ...
        foreach (var key in new List<string>(((IDictionary<string, JsonNode>)o).Keys))
        {
            if (key == "args" && _boxedLocalCallArgs.TryGetValue(o, out var sharedSlots)
                && o[key] is JsonArray args)
            {
                for (int i = 0; i < args.Count; i++)
                {
                    if (sharedSlots.Contains(i) && IsRead(args[i])) continue;
                    if (IsRead(args[i])) args[i] = ReadV();
                    else BoxVarInBody(args[i], x, refName, elem);
                }
                continue;
            }
            ...
            if (nested && (key == "captures" || key == "capValues") && o[key] is JsonArray caps)
            {
