...
static class ContinuationErasure
{
    ...
    {
        // An earlier representation pass may already have moved this declaration onto a CLR slot and captured the
        // whole source type. In that case `owner[slot]` is physical input to this pass, not Kotlin meaning. Recording
        // it again as [KotlinType] would mask the earlier carrier in dll2klib (which correctly prefers the exact
        // KotlinType carrier) and publish a hybrid signature. The existing whole-slot carrier remains authoritative;
        // this pass only changes the physical type below.
    ...
    static bool HasCapturedSourceSurface(JsonObject owner, string fact) =>
        owner[fact] != null || (fact == "retKotlinType"
            ? owner["nullableGenericRet"] != null
                || owner["retSuspendFnType"] != null
                || owner["collIdentityRet"] != null
            : owner["nullableGeneric"] != null
                || owner["suspendFnType"] != null
                || owner["collIdentity"] != null);
