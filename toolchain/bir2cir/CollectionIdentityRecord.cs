...
// stamp time into [KotlinCollectionIdentity(version, bytes)], and facadegen restores `List` vs `MutableList` at every
...
// re-consumed cross-module via facadegen. The stdlib ref surface keeps `kotlin.*` verbatim (no collapse -> no
...
static class CollectionIdentityRecord
{
    ...
    // MutableCollection lowers to IList/ICollection via its OWN alias (not the collapse) and facadegen's reverse-map
    ...
    // IReadOnlyList alias — facadegen restores it without a stamp — and is deliberately NOT recorded).
