...
//
// No MethodImpl accompanies the sibling. Its members (`get_Count`, `get_Item`, `GetEnumerator`) are the same names
// and signatures the mutable face already forced the type to declare as public virtual methods, and the CLR binds
// an interface slot to a matching public virtual method implicitly, per interface. Where a member's physical name
// does NOT match — a Kotlin `size` renamed to `prop_get<size>` — the MethodImpl already exists: the Kotlin
