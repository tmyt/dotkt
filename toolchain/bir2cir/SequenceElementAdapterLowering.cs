...
// Sequence<S> as Sequence<R>. That source cast is valid on erased platforms but cannot alter CLR's
...
static class SequenceElementAdapterLowering
{
    ...
    static void RewriteMarkedMethod(JsonObject method)
    {
        ...
        if (method["static"]?.GetValue<bool>() != true
            ...
            || TypeJson.Read(method["ret"]) != new TypeNode.Fqn(Sequence, new TypeNode[] { resultVariable }))
        ...
        void CollectBoundaries(JsonNode node)
        {
            ...
            {
                if (IsElementBoundary(obj, method["ret"], out _)) boundaries.Add(obj);
        ...
        if (boundaries.Count != 1
            || !IsElementBoundary(boundaries[0], method["ret"], out var operand))
            throw Malformed(method, $"expected one sequence-element cast boundary, found {boundaries.Count}");
        var sourceSequence = (TypeNode.Fqn)TypeJson.Read(operand["sty"]);
            ...
            if (ReferenceEquals(node, boundaries[0]))
                return new JsonObject
                {
                    ...
                    ["type"] = TypeJson.Write(new TypeNode.Fqn(Adapter,
                        new TypeNode[] { sourceSequence.Args[0], resultVariable })),
                    ["argTypes"] = new JsonArray
                    {
                        TypeJson.Write(sourceSequence),
                    },
    ...
    static bool IsElementBoundary(JsonObject node, JsonNode resultType,
