...
// node carries in `resolvedMemberParams`/`argTypes`/`ret` is that MEMBER's declaration, not the caller's Kotlin view of it. Every
...
{
    // A CALL bound to a .NET member: it carries an argument descriptor (`resolvedMemberParams`, or `argTypes` before
    ...
    // stamped declaration itself (`resolvedMemberParams`/`resolvedMemberReturn`) rather than on this set alone.
