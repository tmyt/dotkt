...
.PHONY: all toolchain kotc $(TOOLS) stdlib stdlib-klib stdlib-ref stdlib-rt pack \
        ...
        verify-roundtrip verify-wide-delegates verify-target-universe verify-xfail-policy \
...
verify-core: verify-tests verify-schema verify-sanity verify-lowering verify-msbuild verify-roundtrip verify-wide-delegates verify-target-universe verify-xfail-policy ## every gate except the packaged-SDK release gate
