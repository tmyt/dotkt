...
// dll2klib surfaces the nullable arg/return as Kotlin String?, so a lambda returning null into Func<string?>
// (or reading a String? Action param) compiles only when the byte is honored. Own namespace (DlgNrt).
using System;
namespace DlgNrt {
    public static class Api {
        // A lambda returning null binds here ONLY IF dll2klib surfaces the return as Kotlin `String?` (#150).
