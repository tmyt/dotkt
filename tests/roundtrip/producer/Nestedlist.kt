...
// collapse collided with `MutableList`'s own `IList` alias, so facadegen used to reverse-map the nested `IList` back
// to `MutableList` and surface `Box<MutableList<T>>` — the consumer's `Box<List<String>>` value was then REJECTED.
// bir2cir now stamps [KotlinCollectionIdentity] (the pre-collapse Kotlin type) on each affected slot; facadegen
...
// same-simple-name collision with another package's generic type broke facadegen's re-import of the generic member
