...
// reference-KLIB-projected `ownerType` as the owner and promotes even when the ref index can't attribute it.
...
// — the #199-③ regression: dll2klib must emit `arrOfNulls`'s return as the NAMESPACE-QUALIFIED `kotlinx.genovc.Arr`,
// so the frontend binds `arrOfNulls<String>(3)` to THIS dll's `Arr`, not the twin. Also tests the
// common-fragment promotion (#25 residual).
