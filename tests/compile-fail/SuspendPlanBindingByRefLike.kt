...
// value by its SOURCE ROLE ("the argument 's' of 'suspendPlanByRefLikeLen'"), which the call-evaluation plan carries onto the local
...
suspend fun suspendPlanByRefLikeRelay(n: Int): Int = n + 1
...
fun suspendPlanByRefLikeLen(s: Span<Int>, a: Int, b: Int = a * 10): Int = s.Length + a + b

suspend fun suspendPlanByRefLikeSpanArgumentAcrossSuspension(): Int =
    suspendPlanByRefLikeLen(Span<Int>(arrayOf(1, 2, 3)), suspendPlanByRefLikeRelay(4))

suspend fun main() {
    println(suspendPlanByRefLikeSpanArgumentAcrossSuspension())
}
