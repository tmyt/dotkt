// Cross-file declarations for the CrossFile cross-file battery (from cases/m-c1) — their use sites live in a SIBLING
...
//
//   m-c1  -> CrossFilePoint / CrossFileShape / CrossFileRect : cross-file class + method call, and an open-class `area()`
//            override reached through a non-open `label()` (virtual dispatch across the file boundary).
package crossFileDeclarations

class CrossFilePoint(val x: Int, val y: Int) {
    fun distanceSquared(): Int = x * x + y * y
    fun plus(other: CrossFilePoint): CrossFilePoint = CrossFilePoint(x + other.x, y + other.y)
    fun describe(): String = "($x, $y)"
}

open class CrossFileShape(val name: String) {
    open fun area(): Int = 0
    fun label(): String = "$name area=${area()}"   // non-open label() reaches the overridden area() virtually
}

class CrossFileRect(val w: Int, val h: Int) : CrossFileShape("rect") {
