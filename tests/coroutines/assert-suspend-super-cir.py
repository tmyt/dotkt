...
for state_machine, (receiver_type, helper_suffix, cold_method, generic_arity) in targets.items():
    ...
    if type_node is None:
        raise SystemExit(f"CIR: missing state machine {state_machine}")
    helper_calls = [
        item
    ...
    if len(helper_calls) != 1:
        raise SystemExit(
            f"CIR: {state_machine} has {len(helper_calls)} super helpers, expected exactly 1"
        )
    ...
    if helper_call.get("ownerType") != expected_receiver_owner or helper_call.get("virtual") is not False:
        raise SystemExit(
            f"CIR: {state_machine} does not call its super helper non-virtually: {helper_call!r}"
        )
    if not (
        ...
        raise SystemExit(f"CIR: {state_machine} has an unexpected receiver: {receiver!r}")
    ...
    if len(helpers) != 1:
        raise SystemExit(
            f"CIR: {receiver_type} has {len(helpers)} matching super helpers, expected exactly 1"
        )
    helper_type_params = helpers[0].get("typeParams", [])
    if len(helper_type_params) != generic_arity:
        raise SystemExit(
            f"CIR: {helper_call['method']} has {len(helper_type_params)} generic parameters, "
    ...
    if len(base_calls) != 1:
        raise SystemExit(
            f"CIR: {helper_call['method']} has {len(base_calls)} base cold calls, expected exactly 1"
        )
    call = base_calls[0]
    ...
    if owner != expected_owner or call.get("virtual") is not False or call.get("super") is not True:
        raise SystemExit(
            f"CIR: {helper_call['method']} lost base non-virtual dispatch: "
            f"owner={owner!r}, virtual={call.get('virtual')!r}, super={call.get('super')!r}"
        )
    if call.get("recv") != {"k": "this"}:
        raise SystemExit(f"CIR: {helper_call['method']} has an unexpected base receiver: {call.get('recv')!r}")

print("suspend super-call cold-entry CIR dispatch OK")
