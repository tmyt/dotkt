...
// shared `inlineSuspend`/`InlineSuspend` prefix so their (top-level suspend fun) simple names are UNIQUE across this single assembly —
...
suspend fun inlineSuspendInspTick() { Task.Delay(1).await() }   // a real .NET-async suspension point

suspend fun inlineSuspendInspWork(): Int {
    var n = 0
    for (i in 1..2) {
        inlineSuspendInspTick()
        n += 10
    }
    1.let {
        inlineSuspendInspTick()
...
class InlineSuspendTests {
    ...
    fun suspendCallInNonSuspendInlineArgLambda() {
        assertEquals(21, blockOn { inlineSuspendInspWork() })   // 10 + 10 + 1 = 21
    }
}
