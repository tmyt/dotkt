...
class CorATaskAwaitTests {
    @TestAttribute
    fun cfgAwait_configureAwaitFalseNonGeneric() {
        assertEquals(5, blockOn { corACfgAwait() })   // 5
    }

    @TestAttribute
    fun cfgAwaitGen_configureAwaitFalseGeneric() {
        assertEquals(10, blockOn { corACfgGAwait() })   // 10
    }

    @TestAttribute
    fun coBuild_realTaskDelayAwait() {
        assertEquals(25, blockOn { corABuildTotal() })   // 25
    }

    @TestAttribute
    fun coExc_exceptionAcrossSuspendBoundary() {
        var m1: String? = null
