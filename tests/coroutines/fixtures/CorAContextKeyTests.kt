...
//
// ILVERIFY NOTE: these three carry the SAME runtime-safe formal-only finding as the old verify-il.sh XFAIL_ILVERIFY
...
class CorAContextKeyTests {
    @TestAttribute
    fun coCtxKey_abstractElementCompanionKey() {
        val e = CorACtxkElem()
    ...
    @TestAttribute
    fun coIntercept_interceptorKeyProjection() {
        val i = CorAIceptInterceptor()
        assertEquals(true, i.key === ContinuationInterceptor)   // True
    }

    @TestAttribute
    fun awaitIntercept_scenarioA_interceptorPrecedence() {
    ...
    @TestAttribute
    fun awaitIntercept_scenarioB_defaultCapturingPath() {
    ...
    @TestAttribute
    fun awaitIntercept_scenarioC_configureAwaitFalse() {
        val sink = CorAAwiSink<Int>(EmptyCoroutineContext)
        val tcs = TaskCompletionSource1<Int>()
