...
// covariance finding (GitHub #12, formal-only, runtime-safe). In the per-case bash gate it was a verify-il.sh
...
class CorBSequenceTests {
    @TestAttribute
    fun genseq_genericColdSequence() {
        assertEquals("[5]", corBGsWrap(5).toString())     // [5]
        assertEquals("[hi]", corBGsWrap("hi").toString()) // [hi]
    }

    @TestAttribute
    fun seqyieldall_yieldAllOverloadPick() {
        val s = sequence {
            yield("a")
            yieldAll(listOf("b", "c"))
        }
    ...
    @TestAttribute
    fun genbaseext_externalGenericBaseConcreteArgs() {
