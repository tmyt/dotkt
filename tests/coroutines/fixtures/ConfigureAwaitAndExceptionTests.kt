...
//
// Top-level names are family-prefixed (`configureAwaitCfg`/`configureAwaitCfgG`/`configureAwaitBuild`/`configureAwaitExc`).
...
suspend fun configureAwaitCfgAwait(): Int {
...
suspend fun configureAwaitCfgGAwait(): Int {
...
suspend fun configureAwaitBuildCompute(n: Int): Int {
    Task.Delay(1).await()   // real .NET async suspension
    return n * n
}
suspend fun configureAwaitBuildTotal(): Int {
    val a = configureAwaitBuildCompute(3)   // 9
    val b = configureAwaitBuildCompute(4)   // 16
    return a + b                  // 25
}

// ---- il-coexc: an exception thrown ACROSS a suspended Task boundary propagates to the caller ----------------
suspend fun configureAwaitExcThrowsAfterAwait(): Int {
...
suspend fun configureAwaitExcInner(): Int {
    Task.Delay(1).await()
    throw IllegalStateException("nested")
}
suspend fun configureAwaitExcOuter(): Int {
    val x = configureAwaitExcInner()                // cold call across a suspending frame; fault propagates up
    return x + 1
}
suspend fun configureAwaitExcAwaitsFaultedTask(): Int {
...
class ConfigureAwaitAndExceptionTests {
    ...
    fun configureAwaitFalseNonGeneric() {
        assertEquals(5, blockOn { configureAwaitCfgAwait() })   // 5
    }

    @TestAttribute
    fun configureAwaitFalseGeneric() {
        assertEquals(10, blockOn { configureAwaitCfgGAwait() })   // 10
    }

    @TestAttribute
    fun realTaskDelayAwait() {
        assertEquals(25, blockOn { configureAwaitBuildTotal() })   // 25
    }

    @TestAttribute
    fun exceptionAcrossSuspendBoundary() {
        var m1: String? = null
        try { blockOn { configureAwaitExcThrowsAfterAwait() } } catch (e: IllegalStateException) { m1 = e.message }
        assertEquals("boom", m1)      // former golden: "caught: boom"

        var m2: String? = null
        try { blockOn { configureAwaitExcOuter() } } catch (e: IllegalStateException) { m2 = e.message }
        assertEquals("nested", m2)    // former golden: "caught2: nested"

        var m3: String? = null
        try { blockOn { configureAwaitExcAwaitsFaultedTask() } } catch (e: Throwable) { m3 = e.message }
